...
def find_optimal_lm_mod(df, cutoffs, col_price, prices, test_size = .30, random_state=42, plot=True):
    '''
    INPUT
    ...
    OUTPUT
    df_res - pandas dataframe with 
    r2_scores_train - list of floats of r2 scores on the train data
    lm_model - model object from sklearn
    X_train, X_test, y_train, y_test - output from sklearn train test split used for optimal model
    '''
    cutoff_list, num_feats_list, price_list, r2_score_test_list, r2_score_train_list, results = [], [], [], [], [], dict()
  
    for price in prices:

        print(price)

        df_f = df[df[col_price] < price]

        X = df_f.drop(columns=col_price)
        y = df_f[col_price]
        ...
        for cutoff in cutoffs:
            ...
            num_feats_list.append(reduce_X.shape[1])
...
def find_optimal_lm_mod_2(df, cutoffs, col_price, prices, test_size = .30, random_state=42):
    '''
    INPUT
    df - pandas dataframe, to be used for modeling
    cutoffs - list of ints, cutoff for number of non-zero values in dummy categorical vars
